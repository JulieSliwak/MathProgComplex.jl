...
@testset "WB5 real formulation - dense - order 1" begin
    ...
    @testset "q5min=$q5min, rmeqs=$rmeqs" for (q5min, (dmax, obj_fixedphase, obj_rankrel)) in sols, rmeqs in Set([true, false])

        # info("Working on WB5, no eqs, q5min=$q5min, d=$d")
        problem = buildPOP_WB5(q5min=q5min, rmeqs=rmeqs)

        logpath = joinpath(testfolder, "WB5_q5min_$(q5min)_rankrel")
